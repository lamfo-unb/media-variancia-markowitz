...
* Detalhar um pouco mais os argumentos da função `portfolio.optim(.)`. Por exemplo, ela assume que há venda a descoberto caso o usuário não especifique? "shorts" varia entre TRUE e FALSE ao longo do post - verificar

* Mostrar um caso do output da função `carteira_otima(.)`
