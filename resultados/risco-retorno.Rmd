...
```{r histogramas-risco, echo=FALSE, message=FALSE,eval=TRUE}

library(ggplot2)

# Gerando data
ativo1 <- rnorm(n = 1000, mean = 4, sd = 1)
ativo2 <- rnorm(n = 1000, mean = 4, sd =3)
...
retornos_tidy <- tidyr::gather(retornos, key = 'ativo', value = 'retornos')

# Gráfico
ggplot(retornos_tidy, aes(x = retornos))+
  facet_wrap(~ativo)+
  geom_histogram()+
  labs(
    x = 'Retorno (%)',
